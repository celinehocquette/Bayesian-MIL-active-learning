:- module(metagol_ai,[learn/4, learn/3, pprint/1, pprint_/1, prove_aux/5, prove_deduce/3]).
...
proveall(_,_,_,_,_,M,MaxM,[]) :- succ(MaxM,M), !.
proveall(Atoms,Neg,Sig,N,MaxN,M,MaxM,[Gs|Gs2]) :-
  %format('% clauses: ~d invented predicates: ~d\n',[N,M]),
  target_predicate(Atoms,Name/Arity),!, 
  augmented_sig(Name/Arity,M,Sig,Sig2),
  ((prove(Atoms,Sig2,N,[],Gs1),nproveall(Neg,Sig2,Gs1))
	 -> (copy(Gs1/Sig2,Gs));
	(
	  copy([],Gs))
	),
