...
for k in range(0,n_trials):
    with open('./results/output-%s.pl' %k,'r') as results:
        nHypA[k,:] = np.fromstring(((results.readline()).split('[')[1]).split(']')[0],dtype=float, sep=' ')
        priorA[k,:] = np.fromstring(((results.readline()).split('[')[1]).split(']')[0],dtype=float, sep=' ')
        accuracyA[k,:] = np.fromstring(((results.readline()).split('[')[1]).split(']')[0],dtype=float, sep=' ')
        entropyA[k,:] = np.fromstring(((results.readline()).split('[')[1]).split(']')[0],dtype=float, sep=' ')
        nHypP[k,:] = np.fromstring(((results.readline()).split('[')[1]).split(']')[0],dtype=float, sep=' ')
        priorP[k,:] = np.fromstring(((results.readline()).split('[')[1]).split(']')[0],dtype=float, sep=' ')
        accuracyP[k,:] = np.fromstring(((results.readline()).split('[')[1]).split(']')[0],dtype=float, sep=' ')
        entropyP[k,:] = np.fromstring(((results.readline()).split('[')[1]).split(']')[0],dtype=float, sep=' ')
