...
:- dynamic
    ...
    user:constraint/0,
    user:constraint/2,
    user:deterministic/0,
    user:functional/0,
    user:interpreted/1,
    user:invented_symbol/1,
    user:max_clauses/1,
    user:max_inv_preds/1,
    user:metarule_next_id/1,
    user:min_clauses/1,
    user:print_ordering/0,
    user:primcall/2,
    user:unfold_program/0,
    metabayes:interpreted_bk/2.
...
deduce_atom(Atom,Prog):-
    ...
    findall(sym(P1,A1,_),member(sub(_,P1,A1,_,_),Prog),Sig1),
    sort(Sig1,Sig),
    prove_deduce([Atom1],Sig,Prog).
...
prove_aux(p(_,not,_A,Args,_,_Path),_FullSig,_Sig,_MaxN,N,N,Prog,Prog):-
    Args=[P|Args1],
    length(Args1,L),
    user:body_pred(P/L),
...
prove_aux(p(inv,P,A,_Args,Atom,Path),FullSig,Sig1,MaxN,N1,N2,Prog1,Prog2):-
    ...
    (Recursive==true -> \+memberchk(Atom,Path); true),
    (((user:constraint) -> \+(user:constraint(sub(Name,P,A,MetaSub,PredTypes),Prog1)));(true)),
...
get_option(Option):-user:call(Option), !.
...
user:term_expansion(interpreted(P/A),[L2,user:interpreted(P/A)]):-
    functor(Head,P,A),
    findall((Head:-Body),user:clause(Head,Body),L1),
    maplist(convert_to_interpreted,L1,L2).

convert_to_interpreted((Head:-true),metabayes:(interpreted_bk(HeadAsList,[]))):-!,
    ho_atom_to_list(Head,HeadAsList).
convert_to_interpreted((Head:-Body),metabayes:(interpreted_bk(HeadAsList,BodyList2))):-
    ho_atom_to_list(Head,HeadAsList),
    clause_to_list(Body,BodyList1),
    maplist(ho_atom_to_list,BodyList1,BodyList2).

user:term_expansion(body_pred(P/A),[user:body_pred(P/A),user:(primcall(P,Args):-user:Call)]):-
    functor(Call,P,A),
    Call =.. [P|Args].

user:term_expansion(metarule(MetaSub,Head,Body),Asserts):-
    get_asserts(_Name,MetaSub,Head,Body,_,_PS,Asserts).
user:term_expansion(metarule(Name,MetaSub,Head,Body),Asserts):-
    get_asserts(Name,MetaSub,Head,Body,_,_PS,Asserts).
user:term_expansion((metarule(MetaSub,Head,Body):-MetaBody),Asserts):-
    get_asserts(_Name,MetaSub,Head,Body,MetaBody,_PS,Asserts).
user:term_expansion((metarule(Name,MetaSub,Head,Body):-MetaBody),Asserts):-
    get_asserts(Name,MetaSub,Head,Body,MetaBody,_PS,Asserts).
user:term_expansion((metarule(Name,MetaSub,Head,Body,PS):-MetaBody),Asserts):-
    get_asserts(Name,MetaSub,Head,Body,MetaBody,PS,Asserts).

get_asserts(Name,MetaSub,Head,Body1,MetaBody,PS,[MRule,metarule_init(AssertName,MetaSub,PredTypes,Clause2,Recursive,Path)]):-
    Head = [P|_],
...
assert_clause(Sub):-
    ...
    clause_list_to_clause(ClauseAsList,Head :- Body),
    (\+(clause(Head,Body)) ->
     (assert(user:(Head :- Body))); (true)).
...
retract_program(Prog):-
    maplist(metasub_to_clause_list,Prog,Prog2),
    maplist(clause_list_to_clause,Prog2,Prog3),
    maplist(retract_clause,Prog3).

retract_clause(H:-_):-
    retractall((user:H)).

prim_asserts(P/A,[user:body_pred(P/A), user:(primcall(P,Args):-user:Call)]):-
    functor(Call,P,A),
    Call =.. [P|Args].
